...
use crop::tree::{
    AsSlice, BalancedLeaf, BaseMeasured, Metric, ReplaceableLeaf, SlicingMetric, Summarize, Tree,
};
...
impl<'a> LexemeSlice<'a> {
    /// Splits the slice so that all lexemes before the byte offset
    /// are on the left, and all lexemes after the byte offset are on
    /// the right. If a lexeme contains the byte offset, it is present
    /// in both the left and the right.
    fn split_at_byte_offset(
        &self,
        byte_offset: ByteMetric,
        summary: &Summary,
    ) -> ((Self, Summary), (Self, Summary)) {
        let mut so_far = ByteMetric::measure(summary);
        let (left_end, right_start) = self
            .lexemes
            .0
            .iter()
            .chain(self.lexemes.1)
            .enumerate()
            .find_map(|(index, lexeme)| {
                if byte_offset == so_far {
                    // byte_offset is exactly between two tokens
                    // exclude that lexeme from the left, include it in the
                    // right
                    return Some((index, index));
                }

                let end = so_far + lexeme.bytes;
                if byte_offset < end {
                    // byte_offset is in the middle of a lexeme
                    // include that lexeme in both left and right
            ...
            .expect("byte_offset was in this slice");

        let left = if left_end < self.lexemes.0.len() {
            LexemeSlice::new(&self.lexemes[..left_end], &[] as &[_])
        } else {
            LexemeSlice::new(
                self.lexemes,
                &self.right_lexemes[..left_end - self.lexemes.len()],
            )
        };

        let right = if right_start < self.lexemes.len() {
            LexemeSlice::new(&self.lexemes[..right_start], self.right_lexemes)
        } else {
            LexemeSlice::new(
                &[] as &[_],
                &self.right_lexemes[right_start - self.lexemes.len()..],
            )
        };

        (
            (left, left.summary + summary),
            (right, right.summary + summary),
        )
    }
}
...
impl SlicingMetric<LexemeBuffer> for ByteMetric {
    fn slice_up_to<'a: 'a>(
        slice: LexemeSlice<'a>,
        up_to: ByteMetric,
        summary: &Summary,
    ) -> (LexemeSlice<'a>, Summary) {
        let (left, _) = slice.split_at_byte_offset(up_to, summary);
        left
    }
    ...
    fn slice_from<'a: 'a>(
        ...
        let (_, right) = slice.split_at_byte_offset(byte_offset, summary);
        right
    }
}
